...
public class WebFrameworkDispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (controllerMethod.getParameterCount() > 0) {
				...
				/*
				 * Object arg; Parameter parameter = controllerMethod.getParameters()[0]; if
				 * (parameter.getAnnotations()[0].annotationType().getName()
				 ...
				 * + parameter.getType().getName()); String body = readBytesFromRequest(req);
				 * 
				 * WebFrameworkLogger.log("", "     conteúdo do parâmetro: " + body); arg =
				 * gson.fromJson(body, parameter.getType());
				 * 
				 * WebFrameworkLogger.log("WebFrameworkDispatcherServlet", "Invocar o método " +
				 * controllerMethod.getName() + " com o parâmetro do tipo " +
					...
					for (Annotation annotation : parameter.getAnnotations()) {
						if (annotation.annotationType().getName()
							...
							out.println(gson.toJson(controllerMethod.invoke(controller, arg)));
								...
								.equals("br.com.msfreitas.webframework.annotations.WebFrameworkPathVariable")) {
							WebFrameworkLogger.log("",
									"     Procurando parâmetro da requisição do tipo " + parameter.getType().getName());
							WebFrameworkLogger.log("", "     conteúdo do parâmetro: " + methodParam.getParam() );
	...
	private void injectDependencies(Object controller) throws Exception {
		...
		for (Field attr : controller.getClass().getDeclaredFields()) {
			...
			WebFrameworkLogger.log("WebFrameworkDispatcherServlet",
					"Injetar " + attr.getName() + " do tipo " + attrTipo);
			Object serviceImpl;
